...
namespace ConsoleApp1
{
    class Task2_with_go_to
    {
        static void Main(string[] args)
        {
            ...
            string[] words_arr = new string[99500];
            ...
            string[] less_than_100 = new string[99000];
        ...
        words_less_than_100_loop:
            if (k < length && words_once_arr[k] != null)
            {
                ...
                {
                    less_than_100[LastInsert] = words_once_arr[k];
                    LastInsert++;
                }
                k++;
        ...
        sort_loop:
            if (write < less_than_100.Length && less_than_100[write] != null)
            {
                sort = 0;
            inner_sort_loop:
                if (sort < less_than_100.Length - write - 1 && less_than_100[sort + 1] != null)
                {
                    word_lenth_cur = less_than_100[sort].Length;
                    word_lenth_next = less_than_100[sort + 1].Length;
                ...
                alphabet_check:
                    ...
                    if (less_than_100[sort][counter] > less_than_100[sort + 1][counter])
                    {
                        toSwapWords = true;
                ...
                alphabet_check_end:
                    if (toSwapWords)
                    {
                        string temp = less_than_100[sort];
                        less_than_100[sort] = less_than_100[sort + 1];
                        less_than_100[sort + 1] = temp;
                    }
                    sort++;
            ...
            k = 0;
            int less_than_100_length = less_than_100.Length;
        print_loop:
            if (k < less_than_100_length && less_than_100[k] != null)
            {
                Console.Write("{0} - ", less_than_100[k]);
                int first_dim = 0;
                int second_dim = 0;
            ...
            check_page:
                ...
                check_page_word:
                    ...
                    {
                        if (page_word_arr[first_dim, second_dim] == less_than_100[k])
                        {
